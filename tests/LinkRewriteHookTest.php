...
final class LinkRewriteHookTest extends WP_Mock\Tools\TestCase
{
    ...
    // phpcs:ignore
    public function test_rewritePreviewLink_falls_back_to_publish_path_when_preview_is_blank(): void
    {
        $this->mockSettings([
            'frontend_base_url' => 'https://frontend.example.com',
            'preview_token' => [
        ...
        // 管理画面で preview フィールドを空欄のまま保存した場合 (空文字列) も publish のパスマッピングにフォールバックする
        $result = $hook->rewritePreviewLink('https://wordpress.example.com/?p=123&preview=true', $post);
